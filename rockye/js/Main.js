...
function reset() {
  ...
  var randomNumber = 4 + Math.floor(Math.random() * 3);
  alreadyUsed = [];
...
function createBall(x, y) {
  ...
  var img = new Image();
  img.onload = function () {
    graphics.drawImage(img, 0, 0, xsize, ysize);
  };

  var imgIndex = Math.floor(Math.random() * 18);

  while (alreadyUsed.includes(imgIndex)) {
    imgIndex = Math.floor(Math.random() * 18);
  }

  alreadyUsed.push(imgIndex);
  img.src = `rockyes/rockye${imgIndex}.png`;
